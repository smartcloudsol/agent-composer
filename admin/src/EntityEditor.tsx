...
function SiteContractFields({ payload, update, immutable, help, postTypes, blueprints, localizationProviders }: FieldsProps & { postTypes: ProviderDiscovery["registered_post_types"]; blueprints: ConfigEntity[]; localizationProviders: RegisteredLocalizationProviderManifest[] }) {
  ...
  return <Stack gap="md">
    ...
    <HelpHeading title={__("Governed Media Library ingestion", TEXT_DOMAIN)} topic="site-safety" help={help} />
    ...
    <Switch label={__("Allow Publisher media uploads", TEXT_DOMAIN)} description={__("Lets authenticated MCP Publishers create immediately public Media Library images from bounded base64 bytes or a safe HTTPS URL. Contributors never receive this tool.", TEXT_DOMAIN)} checked={booleanAt(payload, ["design_policy", "remote_media_ingest", "publisher_upload_enabled"])} onChange={(event) => { if (!immutable) update(["design_policy", "remote_media_ingest", "publisher_upload_enabled"], event.currentTarget.checked); }} readOnly={immutable} />
    <SimpleGrid cols={{ base: 1, sm: 2 }}>
      ...
      <MultiSelect label={__("Allowed image types", TEXT_DOMAIN)} description={__("Shared raster MIME allowlist for remote ingestion and Publisher uploads, enforced again from file contents.", TEXT_DOMAIN)} value={stringsAt(payload, ["design_policy", "remote_media_ingest", "allowed_mime_types"])} onChange={(value) => update(["design_policy", "remote_media_ingest", "allowed_mime_types"], value)} data={["image/jpeg", "image/png", "image/webp", "image/avif", "image/gif"]} readOnly={immutable} clearable />
      <NumberInput label={__("Maximum image bytes", TEXT_DOMAIN)} description={__("Shared per-image ceiling from 1 KB through 25 MB; direct MCP payloads remain capped at 12 MiB.", TEXT_DOMAIN)} min={1024} max={26214400} value={numberAt(payload, ["design_policy", "remote_media_ingest", "max_bytes"]) || 12582912} onChange={(value) => update(["design_policy", "remote_media_ingest", "max_bytes"], Number(value) || 0)} readOnly={immutable} />
    </SimpleGrid>
