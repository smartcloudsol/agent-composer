...
final class ExecutionAbilityAliases {
	...
	private function description( string $target ): string {
		...
		if ( 'update-own-draft' === $target ) {
			return $this->abilities->is_rendered_preview_required()
				? 'Updates an assigned Composer draft. After the final successful update, MUST call smartcloud-agent-composer/get-rendered-preview with the returned post_id, modified_gmt, and revision and let the inline rendered HTML preview be delivered before reporting completion. If assignment_source is published-update-proposal, validate the completed proposal, call that preview tool, then MUST call smartcloud-agent-composer/submit-content-proposal with the same concurrency tokens and the exact rendered_preview_token returned by the preview. Do not submit before previewing, leave a completed proposal in working state, or report it as ready before submission succeeds.'
				: 'Updates an assigned Composer draft. Rendered HTML preview after the final successful update remains the recommended default. If the user explicitly asks to skip it, a normal draft may be completed without preview and a published-update-proposal must still be validated and submitted with the freshest concurrency tokens, omitting rendered_preview_token. Do not leave a completed proposal in working state or report it as ready before submission succeeds.';
		}
