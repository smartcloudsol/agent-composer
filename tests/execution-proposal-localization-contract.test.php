...
$assert( str_contains( $proposal, "\$result['change_details']" ) && str_contains( $proposal, "'path'        => \$path" ) && str_contains( $proposal, "'before'      => \$old_value" ) && str_contains( $proposal, "'after'       => \$new_value" ), 'Human proposal review must expose field-level paths and before/after values instead of collapsing all structured fields into meta.' );
$assert( str_contains( $proposal, "get_registered_meta_keys( 'post', \$post_type )" ) && str_contains( $proposal, "\$registration['label']" ) && str_contains( $proposal, "\$registration['description']" ), 'Structured proposal changes must reuse registered human and AI field guidance.' );
$assert( str_contains( $proposal, 'proposal_meta_input' ) && str_contains( $proposal, "metadata_exists( 'post', \$source_id" ), 'Proposal creation must not materialize absent registered fields merely by cloning them.' );
$assert( substr_count( $proposal, 'meta_values_equal' ) >= 2 && str_contains( $proposal, 'normalized_meta_comparison_value' ), 'Review and merge verification must treat absent and empty registered collection fields as the same semantic value.' );
...
$assert( str_contains( $proposal, 'resolve_for_blueprint' ) && str_contains( $drafts, 'resolve_for_blueprint' ), 'Published proposal creation and content inspection must use the Blueprint-authored language fallback.' );
$assert( str_contains( $localization, "'language_source'" ) && str_contains( $localization, "'blueprint'" ) && str_contains( $localization, 'CONTENT_LANGUAGE_META' ), 'Monolingual Blueprint language must outrank the WordPress UI locale when no localization provider is active.' );
$assert( str_contains( $proposal, 'Draft_Service::CONTENT_LANGUAGE_META => $content_language' ), 'Published-content proposals must preserve the localized source language for draft inspection and rendered previews.' );
